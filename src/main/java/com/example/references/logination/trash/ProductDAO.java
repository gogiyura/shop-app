...
import com.example.references.Utils;
...
public class ProductDAO {
    ...
    public static Product findProductById(long id) throws SQLException{
        ...
        try (Connection con = getConnection()) {
            PreparedStatement pst = con.prepareStatement(SQL_INSERT_PRODUCT);
            pst.setLong(1, id);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
    ...
    public static long addOrUpdateProduct(Product product) {
        ...
        try (Connection con = getConnection();
             PreparedStatement pst = con.prepareStatement(
                     productId > 0 ? SQL_UPDATE_PRODUCT : SQL_INSERT_PRODUCT,
